...
from bot.commands.categories import CATEGORY_LABELS, label_for
...
class CategoryConsentView(discord.ui.View):
    ...
    def render(self) -> str:
        chosen = (
            "、".join(label_for(c) for c in self.selected_categories)
            if self.selected_categories
            else "（未選択）"
        )
        return (
            f"通報対象: `{self.target_id}`（{self.target_username}）\n"
            f"選択中のカテゴリ: {chosen}\n\n"
            f"{CONFIRMATION_TEXT}"
        )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        categories_label = "、".join(label_for(c) for c in self.selected_categories)
        embed = discord.Embed(title="新しい通報", color=discord.Color.orange())
        embed.add_field(name="報告者", value=f"{self.reporter.mention}（`{self.reporter.id}`）", inline=False)
        embed.add_field(
            name="対象ユーザー", value=f"{self.target_username}（`{self.target_id}`）", inline=False
        )
...
class ApprovalView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        try:
            existing = await get_existing_record(report.target_id)
            ...
            await commit_user_record(
                merged, commit_message=f"report: list {report.target_id} (report #{report.id})"
            )
        ...
        try:
            await reporter.send(
                f"通報（対象: `{report.target_id}`）が承認され、通報リストに反映されました。ご協力ありがとうございます。"
            )
    ...
    async def finalize_rejection(self, interaction: discord.Interaction, reason: str) -> None:
        ...
        try:
            await reporter.send(
                f"通報（対象: `{report.target_id}`）は却下されました。\n理由: {reason}"
            )
        except discord.Forbidden:
            pass
